...
use crate::chesspieces;
use crate::piece::Color::*;
use crate::piece::{Color, Piece, Suit};
...
impl GameState {
    ...
    fn standard_board() -> Board {
        ...
        board[0][0] = Field::Piece(chesspieces::BL_ROOK);
        board[7][0] = Field::Piece(chesspieces::WH_ROOK);
        board[0][7] = Field::Piece(chesspieces::BL_ROOK);
        board[7][7] = Field::Piece(chesspieces::WH_ROOK);
        board[0][1] = Field::Piece(chesspieces::BL_KNIGHT);
        board[7][1] = Field::Piece(chesspieces::WH_KNIGHT);
        board[0][6] = Field::Piece(chesspieces::BL_KNIGHT);
        board[7][6] = Field::Piece(chesspieces::WH_KNIGHT);
        board[0][2] = Field::Piece(chesspieces::BL_BISHOP);
        board[7][2] = Field::Piece(chesspieces::WH_BISHOP);
        board[0][5] = Field::Piece(chesspieces::BL_BISHOP);
        board[7][5] = Field::Piece(chesspieces::WH_BISHOP);
        board[0][4] = Field::Piece(chesspieces::BL_KING);
        board[7][4] = Field::Piece(chesspieces::WH_KING);
        board[0][3] = Field::Piece(chesspieces::BL_QUEEN);
        board[7][3] = Field::Piece(chesspieces::WH_QUEEN);
    ...
    pub fn performe_move_index(&mut self, index: usize) {
        ...
        let mov = self.moves.get_mut(&color).unwrap().remove(index);
    ...
    fn produce_pawn_moves(&self, pos: Point, color: Color, moves: &mut Vec<Move>) {
        ...
        let possible_charge = Move::new(
            pos,
            Point {
                x,
                y: y + 2 * Self::directition(color),
            },
            "pawn charge",
        ); //  only from start point
        ...
        // verify moves
        if self.verify_on_board(possible_move.to)
            && !self.field_contains_opponent(possible_move.to, color)
        {
            // no opponent figure directly infront of pawn
            moves.push(possible_move);

            if self.verify_on_board(possible_charge.to)
                && (pos.y - Self::directition(color)) % 7 == 0
            // pawns may only charge from their start os
            {
        ...
        // verify captures
        if self.verify_on_board(possible_capture_l.to)
            && self.field_contains_opponent(possible_capture_l.to, color)
        {
            moves.push(possible_capture_l)
        };
        if self.verify_on_board(possible_capture_r.to)
            && self.field_contains_opponent(possible_capture_r.to, color)
        {
    ...
    fn produce_knight_moves(&self, pos: Point, color: Color, moves: &mut Vec<Move>) {
        for i in 0..4 {
            ...
            match self.get_field(mov.to) {
                ...
                Field::Piece(piece) => if piece.color != color {
    ...
    fn produce_rook_moves(&self, pos: Point, color: Color, moves: &mut Vec<Move>) {
        for &(x_dir, y_dir) in &[(0, 1), (1, 0), (0, -1), (-1, 0)] {
            self.produce_moves_for_dir(pos, color, moves, x_dir, y_dir, self.board.len() as i8)
        }
    }
    ...
    fn produce_king_moves(&self, pos: Point, color: Color, moves: &mut Vec<Move>) {
        ...
        for &(x_dir, y_dir) in &[(0, 1), (1, 0), (0, -1), (-1, 0)] {
            self.produce_moves_for_dir(pos, color, moves, x_dir, y_dir, 1)
        }
    }

    fn produce_moves_for_dir(
        &self,
        pos: Point,
        color: Color,
        moves: &mut Vec<Move>,
